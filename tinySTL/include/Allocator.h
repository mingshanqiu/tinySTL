...
namespace tinySTL {
	...
	public:
		...
		static void *reallocate(void *p, std::size_t m) {
	...
	private:
		...
		static void *reallocHandle(std::size_t) {
			throw std::runtime_erroe("none memory");
		}
	};
	...
	private:
		...
		static void *refill(std::size_t n) {
			int nfreelists = 20;
			char *chunk = chunk_alloc(n, nfreelist);
			if (nfreelists == 1)
				return chunk;
			else {
				freeList_[FREELIST_INDEX(n)] = (FreelistNode *)(chunk + n);
				FreeListNode *curr = freeList_[FREELIST_INDEX(n)];
				for (int i = 0; i < nfreelistd - 1; ++i) {
					if (i != nfreelists - 2)
						curr->next = (FreeListNode *)((char *)(curr)+n);
					else
						curr->next = nullptr;
				}
				return chunk;
	...
	typedef MallocAlloc<0> Allocator;
	...
	OutputIterator uninitializedCopy(InputIterator forn, InputIterator back, OutputIterator target) {
		typedef typename iterator_traits<OutputIterator>::value_type valueType;
		const auto n = back - front;
		for (int i = 0; i < n; ++i)
			construct(&*(target + i), valueType());
		std::copy(frontm back, target);
		return target;
	}
}